...
데코레이터 패턴은 상속이 아닌 위임을 하는 방식으로 확장해 나간다.

기존 기능에서 새로운 기능을 추가해주는 역할을 한다. 각 확장 기능들의 구현이 별도의 클래스로 분리되기 때문에, 각 확장 기능 및 원래 기능을 서로 영향 없이 변경할 수 있게 한다.

### 프록시 패턴 - Proxy

프록시 패턴은 실제 객체를 대신하는 프록시 객체를 사용해서 실제 객체의 생성이나 접근 등을 제어할 수 있게 해 주는 패턴이다.

### 어댑터 패턴 - Adapter

어댑터 패턴은 클라이언트가 요구하는 인터페이스와 재사용하려는 모듈의 인터페이스가 일치하지 않을 때 사용할 수 있는 패턴이다. 즉, 사용할 수 없는 인터페이스를 연결해주는 역할을 한다.

### 옵저버 패턴 - Observer

한 객체의 상태 변화를 정해지지 않은 다른 객체에 통지하고 싶을 때 사용되는 패턴이 옵저버 패턴이다.
